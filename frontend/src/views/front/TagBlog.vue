  ...
  methods: {
    async loadMore() {
      ...
      try {
        ...
        const response = await axios.get(`http://localhost:8080/api/v1/public/tag-blog/${id}`, {
          params: {
    ...
    getSingleTag: async function(){
      try {
        let id = this.$route.params.id;
        await http().get(`http://localhost:8080/api/v1/public/tag/${id}`).then(res => {
    ...
    <div class="category_blog_main">
      <div class="category_blog_container">
        ...
        <div class="blog" v-if="blogs.length > 0">
          <h2 class="h2">Latest Blog By {{tag.name}}</h2>
